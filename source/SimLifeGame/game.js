class SimLifeGame {
    constructor() {
        this.gameState = {
            ...
            pspSystem: null, // PSP tracking system
            cashFlowHistory: [],
            gameOver: false,
            gameStarted: false
        };
    ...
    startGame() {
        ...
        // PSP System logging and UI setup
        console.log('DEBUG: startGame - checking PSP system:', this.gameState.pspSystem);
        console.log('DEBUG: startGame - profession ID:', this.gameState.professionId);
        if (this.gameState.pspSystem) {
            console.log('DEBUG: PSP system found, adding logs and button');
            this.log('🔧 PSP System Initialized!', 'success');
            this.log('📊 Personal Software Process tracking enabled', 'info');
            this.log(`Initial Productivity Score: ${this.gameState.pspSystem.productivityScore}/100`, 'info');
            this.log(`Initial Quality Score: ${this.gameState.pspSystem.qualityScore}/100`, 'info');
            this.log('Available PSP commands: psp dashboard, psp project create, psp consult', 'info');
            this.addPSPButton();
        } else {
            console.log('DEBUG: No PSP system found, skipping PSP setup');
        }
    ...
    initializeProfession(professionId) {
        ...
        // Initialize PSP System for PSP Software Developer
        console.log('DEBUG: Initializing profession:', professionId);
        if (professionId === 'software_developer_psp') {
            console.log('DEBUG: PSP profession detected, initializing PSP system');
            this.gameState.pspSystem = {
                enabled: true,
                totalProjects: 0,
                completedProjects: 0,
                totalTasks: 0,
            ...
            console.log('DEBUG: PSP system initialized:', this.gameState.pspSystem);
        } else {
            console.log('DEBUG: Not PSP profession, PSP system not initialized');
        }
    }
    ...
    async loadProfessions() {
        ...
        // Debug: Check if PSP profession is loaded
        if (this.professions['software_developer_psp']) {
            console.log('DEBUG: PSP profession loaded successfully:', this.professions['software_developer_psp'].title);
        } else {
            console.log('DEBUG: PSP profession NOT found in loaded professions');
        }
    }
    ...
    // PSP System Methods
    addPSPButton() {
        console.log('DEBUG: addPSPButton called');
        // Find the game controls area and add PSP Dashboard button
        // Look for the Game Controls panel specifically
        const gamePanels = document.querySelectorAll('.game-panel');
        let controlsArea = null;
        
        gamePanels.forEach(panel => {
            const header = panel.querySelector('h3');
            if (header && header.textContent.includes('Game Controls')) {
                controlsArea = panel;
                console.log('DEBUG: Found Game Controls panel');
            }
        });
        
        if (!controlsArea) {
            console.log('DEBUG: Game Controls panel not found, trying alternative selectors');
            controlsArea = document.querySelector('.game-controls') || document.querySelector('.controls-container');
        }
        
        if (controlsArea) {
            console.log('DEBUG: Found controls area, adding PSP button');
            const pspButton = document.createElement('button');
            pspButton.className = 'btn psp-dashboard-btn';
            pspButton.innerHTML = '📊 PSP Dashboard';
            pspButton.style.cssText = `
            ...
            pspButton.addEventListener('click', () => this.showPSPDashboard());
            pspButton.addEventListener('mouseenter', function() {
                this.style.transform = 'translateY(-2px)';
                this.style.boxShadow = '0 6px 20px rgba(102, 126, 234, 0.4)';
            });
            pspButton.addEventListener('mouseleave', function() {
                this.style.transform = 'translateY(0)';
                this.style.boxShadow = '0 4px 15px rgba(102, 126, 234, 0.3)';
            });
            
            controlsArea.appendChild(pspButton);
            console.log('DEBUG: PSP button added successfully');
        } else {
            console.log('DEBUG: Could not find controls area to add PSP button');
            console.log('DEBUG: Available elements:', document.querySelectorAll('.game-panel').length, 'game panels found');
        }
    }
    
    showPSPDashboard() {
        const psp = this.gameState.pspSystem;
        if (!psp) return;
        ...
        const successRate = psp.totalProjects > 0 ? Math.floor((psp.completedProjects / psp.totalProjects) * 100) : 85;
        
        modal.innerHTML = `
            <div style="padding: 20px; border-bottom: 2px solid #f0f0f0; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border-radius: 12px 12px 0 0;">
                <h2 style="margin: 0; display: flex; justify-content: space-between; align-items: center;">
                    Personal Software Process Dashboard
                    <span style="cursor: pointer; font-size: 24px;" onclick="this.closest('.psp-backdrop').remove()">×</span>
                </h2>
            </div>
            
            <div style="padding: 20px;">
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 15px; margin-bottom: 20px;">
                    <div style="background: #f8f9fa; border-radius: 8px; padding: 15px; text-align: center; border-left: 4px solid #4CAF50;">
                        <h3 style="margin: 0 0 10px 0; color: #333; font-size: 0.9em;">PRODUCTIVITY</h3>
                        <div style="font-size: 2em; font-weight: bold; color: #333;">${psp.productivityScore}/100</div>
                    </div>
                    
                    <div style="background: #f8f9fa; border-radius: 8px; padding: 15px; text-align: center; border-left: 4px solid #2196F3;">
                        <h3 style="margin: 0 0 10px 0; color: #333; font-size: 0.9em;">QUALITY</h3>
                        <div style="font-size: 2em; font-weight: bold; color: #333;">${psp.qualityScore}/100</div>
                        <div style="font-size: 0.8em; color: #666;">${psp.defectRate.toFixed(1)} defects/1000 LOC</div>
                    </div>
                    
                    <div style="background: #f8f9fa; border-radius: 8px; padding: 15px; text-align: center; border-left: 4px solid #FF9800;">
                        <h3 style="margin: 0 0 10px 0; color: #333; font-size: 0.9em;">PROJECTS</h3>
                        <div style="font-size: 1.5em; font-weight: bold; color: #333;">${psp.completedProjects}/${psp.totalProjects}</div>
                        <div style="font-size: 0.8em; color: #666;">${successRate}% success rate</div>
                    </div>
                    
                    <div style="background: #f8f9fa; border-radius: 8px; padding: 15px; text-align: center; border-left: 4px solid #9C27B0;">
                        <h3 style="margin: 0 0 10px 0; color: #333; font-size: 0.9em;">CONSULTING</h3>
                        <div style="font-size: 1.2em; font-weight: bold; color: #4CAF50;">$${psp.consultingIncome.toLocaleString()}/month</div>
                    </div>
                </div>
                
                <div style="display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 20px;">
                    <button onclick="game.executePSPCommand('psp project create')" style="background: #667eea; color: white; border: none; border-radius: 6px; padding: 8px 16px; cursor: pointer;">New Project</button>
                    <button onclick="game.executePSPCommand('psp task add')" style="background: #667eea; color: white; border: none; border-radius: 6px; padding: 8px 16px; cursor: pointer;">Add Task</button>
                    <button onclick="game.executePSPCommand('psp metric record')" style="background: #667eea; color: white; border: none; border-radius: 6px; padding: 8px 16px; cursor: pointer;">Record Time</button>
                    <button onclick="game.executePSPCommand('psp consult')" style="background: #667eea; color: white; border: none; border-radius: 6px; padding: 8px 16px; cursor: pointer;">Consulting</button>
                </div>
                
                <div>
                    <h4 style="margin: 0 0 10px 0; color: #333;">Recent Activity</h4>
                    <div style="font-size: 0.9em; color: #555;">
                        <div>✅ System initialized successfully</div>
                        <div>📊 Tracking productivity and quality metrics</div>
                        <div>💡 Use PSP commands to record development activity</div>
                        <div>🎯 Build up your productivity score for consulting opportunities</div>
                    </div>
                </div>
            </div>
        `;
        
        backdrop.className = 'psp-backdrop';
    ...
    executePSPCommand(command) {
        const psp = this.gameState.pspSystem;
        if (!psp) {
            this.log('PSP System not available for this profession.', 'error');
            return;
        }
        
        if (command === 'psp project create') {
            const projectName = prompt('Enter project name:') || 'New Project';
            psp.totalProjects++;
            this.log(`🚀 Created project: ${projectName}`, 'success');
            this.log(`Total projects: ${psp.totalProjects}`, 'info');
        } else if (command === 'psp task add') {
            const taskName = prompt('Enter task name:') || 'New Task';
            const hours = prompt('Estimated hours:') || '4';
            psp.totalTasks++;
            this.log(`📋 Added task: ${taskName} (${hours}h estimated)`, 'success');
            this.log(`Total tasks: ${psp.totalTasks}`, 'info');
        } else if (command === 'psp metric record') {
            const hours = parseFloat(prompt('Hours worked:') || '1');
            psp.totalHoursTracked += hours;
            psp.productivityScore = Math.min(100, psp.productivityScore + 1);
            this.log(`⏱️ Recorded ${hours} hours of development time`, 'success');
            this.log(`Productivity score increased to ${psp.productivityScore}`, 'info');
        } else if (command === 'psp consult') {
            if (psp.productivityScore >= 70 && psp.qualityScore >= 65) {
                const income = this.randomBetween(500, 2000);
                psp.consultingIncome += income;
                this.gameState.portfolio.cash += income;
                this.log(`💰 PSP Consulting completed! Earned $${income}`, 'success');
                this.log(`Monthly consulting income: $${psp.consultingIncome}`, 'info');
                this.updateUI();
            } else {
                this.log('❌ Consulting requires Productivity ≥70 and Quality ≥65', 'error');
                this.log(`Current: Productivity ${psp.productivityScore}, Quality ${psp.qualityScore}`, 'info');
            }
        }
        
        // Update any open dashboard
        const existingDashboard = document.querySelector('.psp-backdrop');
        if (existingDashboard) {
            existingDashboard.remove();
            this.showPSPDashboard();
        }
    }
}
